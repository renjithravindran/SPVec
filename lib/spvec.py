...
def coocs_worker(chunk_id, windowsize, modeltype, vocabsize):
    ...
    coo_mat = coo_matrix((coocs_v, (coocs_i, coocs_j)), shape=(
        vocabsize, vocabsize), dtype=np.dtype('u4'))
...
def pmi_worker2(chunk_id, vocabsize):
    ...
    for i, j in chunks_[chunk_id]:
        ...
        pmi = log2(ij_cooc + 1) - log2(i_cooc + 1) - \
            log2(j_cooc + 1) + log2(total_coocs_ + 1)
        rows.append(i)
        cols.append(j)
        pmi_vals.append(pmi)
